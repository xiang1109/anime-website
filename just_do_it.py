...
# 写入SQL文件
with open('d:\\code\\trae-ai\\anime_data_5000.sql', 'w', encoding='utf-8') as f:
    f.write(sql_script)
...
# 上传到服务器
sftp = ssh.open_sftp()
sftp.put('d:\\code\\trae-ai\\anime_data_5000.sql', '/tmp/anime_data_5000.sql')
sftp.close()
print("SQL文件已上传到服务器")

# 执行SQL
print("\n=== 3. 执行SQL（这可能需要几分钟） ===")
out, err = run_command(ssh, "mysql -u root -p'Xinmima1109' < /tmp/anime_data_5000.sql 2>&1")
print("SQL执行完成")
