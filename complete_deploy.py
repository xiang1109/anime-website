...
def deploy_to_server():
    ...
    try:
        ...
        print("\n上传数据获取脚本...")
        sftp = ssh.open_sftp()
        sftp.put('simple_anime_fetcher.py', '/opt/anime-website/simple_anime_fetcher.py')
        sftp.close()
...
def update_anime_data(ssh):
    """更新动漫数据"""
    print("\n" + "="*70)
    print("步骤 3: 更新动漫数据（生成5000条）")
    print("="*70)
    
    # 运行数据生成脚本
    print("\n开始生成动漫数据（这可能需要一些时间）...")
    out, err = run_command(ssh, "cd /opt/anime-website && python3 simple_anime_fetcher.py 2>&1", show=True)
...
def verify_deployment(ssh):
    ...
    # 检查分页
    out, err = run_command(ssh, "curl -s 'http://localhost/api/anime?page=1&limit=10'")
    print("\n分页测试:", out[:200])
