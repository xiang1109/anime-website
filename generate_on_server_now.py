...
for i in range(5000):
    ...
    if (i + 1) % 100 == 0:
        print(f"已生成 {i+1} 条记录")
        sql = "INSERT INTO anime_db.animes (title, title_jp, description, cover_image, episodes, status, release_year, studio, genre, average_rating, rating_count, created_at) VALUES\n" + ",\n".join(values) + ";"
        run_command(ssh, f"mysql -u root -p'Xinmima1109' -e \"{sql}\" 2>&1", show=False)
        values = []
        print(f"已插入 {i+1} 条记录")
