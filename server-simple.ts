...
app.put('/api/admin/animes/:id', authenticateToken, isAdmin, async (req, res) => {
  try {
    ...
    if (episodes !== undefined) {
      ...
      updateValues.push(Number(episodes));
    }
    if (status !== undefined) {
    ...
    if (release_year !== undefined) {
      ...
      updateValues.push(release_year ? Number(release_year) : null);
    }
    if (studio !== undefined) {
    ...
    if (average_rating !== undefined) {
      ...
      updateValues.push(parseFloat(average_rating as any) || 0);
    }
    if (rating_count !== undefined) {
      updateFields.push('rating_count = ?');
      updateValues.push(Number(rating_count) || 0);
    }
...
// ==================== 管理员：获取单个动漫详情 ====================
app.get('/api/admin/animes/:id', authenticateToken, isAdmin, async (req, res) => {
  try {
    const animeId = parseInt(req.params.id || '0');
    if (isNaN(animeId) || animeId <= 0) {
      return res.status(400).json({ success: false, message: '无效的动漫ID' });
