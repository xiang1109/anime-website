...
# 1. 在后端添加对/api/anime的支持（重定向到/api/animes）
print("\n=== 1. 更新后端代码 ===")
...
app.get('/api/admin/animes', authenticateToken, isAdmin, async (req, res) => {
  try {
    const [animes] = await pool.query('SELECT * FROM animes ORDER BY id DESC') as any[];
...
app.post('/api/admin/animes', authenticateToken, isAdmin, async (req, res) => {
  try {
    ...
    const [newAnimes] = await pool.query(
      'SELECT * FROM animes WHERE id = ?',
...
print("="*60)
print("\n现在支持两种API路径：")
print("1. /api/anime (单数) - 兼容前端代码")
print("2. /api/animes (复数) - RESTful标准")
print("\n请在浏览器中访问：")
